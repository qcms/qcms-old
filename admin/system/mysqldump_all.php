...
if($MAIN->CheckGet("action", "test"))
{
    ...
    {
        s_add_message($MAIN->QueryStringWithoutParams(), "Проверка подключения (".DATABASE_USER."@".DATABASE_SERVER.") к БД (".DATABASE_NAME.") выполнена успешно!");
    }
    else
    {
        if(!$link)
...
if($MAIN->CheckGet("action", "backup"))
{
    ...
    if($filename)
    {
        s_add_message($MAIN->QueryStringWithoutParams(), "Резервная копия выполнена успешно!<br />Файл '{$filename}' успешно добавлен.");
...
if($MAIN->CheckGet("action", "delete") && $MAIN->CheckGetExists("f"))
{
    ...
    if(preg_match('/^[\w\d-]+\.sql$/is', $entry))
    {
        ...
        s_add_message($MAIN->QueryStringWithoutParams(), "Файл '{$entry}' успешно удален.");
    }
...
s_reset_messages($MAIN->QueryStringWithoutParams());
...
function admin_show_messages()
{
    ...
    $messages = s_get_messages_array($MAIN->QueryStringWithoutParams());
