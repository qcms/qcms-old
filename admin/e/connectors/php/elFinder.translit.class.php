...
class elFinder {
	...
	{
		$name = $this->_rusutf2translit($_GET['name']);
	...
	private function _mkfile()
	{
		...
		$this->_logContext['file'] = $dir.DIRECTORY_SEPARATOR.$name; 
		...
		for ($i=0, $s = count($_FILES['upload']['name']); $i < $s; $i++) { 
			if (!empty($_FILES['upload']['name'][$i])) {
				...
				if ($_FILES['upload']['error'][$i] > 0) {
					...
					$file = $dir.DIRECTORY_SEPARATOR.$this->_rus2translit($_FILES['upload']['name'][$i]);
	...
	private function _rusutf2translit($str)
	{
		$str = iconv("UTF-8", "WINDOWS-1251", urldecode($str));
