...
//-----------------------------------------------------------------------------
// function add message in session object
function s_add_message($source, $message)
{
    if( !isset($_SESSION["messages"][$source])
        || !is_array($_SESSION["messages"][$source]) )
    {
        // create messages for this source
        $_SESSION["messages"][$source] = array();
    }
    // add message for this source
    array_push($_SESSION["messages"][$source], $message);
}


//-----------------------------------------------------------------------------
// function returns var array for source
function s_get_messages_array($source)
{
    ...
    return $_SESSION["messages"][$source];
}
...
//-----------------------------------------------------------------------------
// function reset information about messages for specified source
// stored in session object
function s_reset_messages($source)
{
    $_SESSION["messages"][$source] = array();
}
