...
$entity = array(
  ...
  "user" => array(
    "name" => array("Пользователи сайта"),
    "admin" => array(
      "all" => array(
        "url_template" => "user_all.php",
        "file" => "user_all.php",
        "name" => array("Пользователи сайта")
      ),
      "edit" => array(
        "url_template" => "user_edit.php?id=[id]",
        "file" => "user_edit.php",
        "name" => array("Редактирование Пользователя")
      ),
      "delete" => array(
        "url_template" => "user_edit.php?id=[id]&action=delete",
        "file" => "user_edit.php",
        "name" => array("Удалить")
      ),
    ),
...
//      "user_datetime_burn" => array(
//        "name" => array("Дата/время Рождения"),
      ...
      "user_type" => array(
        ...
        "params" => array( // параметры поля
          ...
          "select" => array( // параметры поля типа select
            "size" => "1", // size для поля типа select
            "multiselect" => "false", // возможность множественного выбора для поля типа select
            "values" => "user_types", // значения в переменной  из "массивы предопределенных значений"
          )
        )
      ),      
      "user_datetime_create" => array(
        "name" => array("Дата/время регистрации"),
        "type" => "datetime", // тип поля string, password, text, number, datetime, checkbox, select, image, sound, video, file
        "params" => array( // параметры поля
          "default" => array()
          //"value" => array("NOW")
...
//            "multiselect" => "false", // возможность множественного выбора для поля типа select
//            "table" => array(
//              "name" => "user_child", // имя таблицы
//              "field" => "user_child_id", // имя поля для значений
//              "namefield"=>"user_child_name", // имя поля для имен
//							"where" => "user_id = [identity]",		// фильтрация
//    					//"orderfield"=>"tale_order ASC" // имя поля упорядочивания
//            )
//          )
//        )
//      ),
      "user_admin_comment" => array(
        "name" => array("Комментарий админа"),
      ...
      "user_datetime" => array(
		...
		"order" => array("user_email"=>"ASC"), // поля упорядочивания в админке
	),
);
